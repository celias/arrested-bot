...
function cat(cp) {
  ...
  let index = (0, _core.findUnicodeRangeIndex)(cp, _grapheme_data.grapheme_ranges, bmpCursor);
  return index < 0 ? 0 : _grapheme_data.grapheme_ranges[index][2];
}
;
...
function isBoundary(catBefore, catAfter, risCount, emoji, incb) {
  ...
  // GB7 - (LV | V) x (V | T)
  if ((catBefore === 7 || catBefore === 13) && (catAfter === 13 || catAfter === 12)) {
    return false;
  }

  // GB8 - (LVT | T) x T
  if ((catBefore === 8 || catBefore === 12) && catAfter === 12) {
    return false;
  }
